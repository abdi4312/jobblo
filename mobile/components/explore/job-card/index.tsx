  ...
  return (
    ...
    >
      ...
      >
        ...
        {item.categories && item.categories.length > 0 && (
          <View className="absolute bottom-3 left-3 bg-[#E68A2E] px-3 py-1 rounded-full">
            <Text className="text-white text-[10px] font-medium">{item.categories[0]}</Text>
          </View>
        )}
      </View>

      {/* Title */}
      <Text className="text-custom-black text-base font-medium mb-1 px-1" numberOfLines={1}>
        {item.title}
      </Text>
      ...
      <View className="flex-row flex-wrap items-end justify-end px-1">
        <Text className="text-custom-black text-base font-semibold">NOK {item.price}</Text>
      </View>
    </TouchableOpacity>
  );
};
