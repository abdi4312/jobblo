...
export const addContributors = async (listId: string, userIds: string[]): Promise<List> => {
  const response = await api.post(`/lists/${listId}/contributors`, { userIds });
  return response.data;
};

export const removeContributor = async (listId: string, userId: string): Promise<List> => {
