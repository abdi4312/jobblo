...
import { useProviderOrder, useCreateProviderReviewMutation, useDeleteProviderEvidenceMutation, useMarkReadyForReviewMutation, useOpenProviderDisputeMutation, useProviderChecklistMutation, useProviderDispute, useProviderEvidenceMutation, useProviderOrderReviews, useStartProviderJobMutation } from '../../../../src/hooks/useProviderOrder';
...
const DISPUTE_REASONS = [
  { value: 'payment_issue', label: 'Problem med betaling' },
  { value: 'scope_change', label: 'Endring i oppdraget' },
  { value: 'customer_unresponsive', label: 'Oppdragsgiver svarer ikke' },
...
export default function ProviderOrderScreen() {
  ...
  const disputeMutation = useOpenProviderDisputeMutation(orderId ?? '');
  ...
  const submitDispute = () => {
    if (!reasonCategory || disputeTitle.trim().length < 5 || disputeDescription.trim().length < 20) { Alert.alert('Sjekk feltene', 'Velg en årsak, skriv en tittel på minst 5 tegn og en beskrivelse på minst 20 tegn.'); return; }
    disputeMutation.mutate({ reasonCategory, title: disputeTitle.trim(), description: disputeDescription.trim() }, { onSuccess: () => { setDisputeOpen(false); setReasonCategory(''); setDisputeTitle(''); setDisputeDescription(''); }, onError: (e) => Alert.alert('Kunne ikke opprette tvist', errorMessage(e, 'Prøv igjen.')) });
  };
  ...
  const canDispute = isProvider && !activeDispute && ['paid', 'in_progress', 'ready_for_review'].includes(status);
  ...
  return <SafeAreaView className="flex-1 bg-[#EFF0EA"><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
    <View className="mb-5 flex-row items-center justify-between"><Pressable onPress={() => router.back()} className="flex-row items-center gap-2"><ArrowLeft size={16} color="#63665F" /><Text className="text-[0.8125rem] text-[#63665F]">Tilbake</Text></Pressable>{typeof orderId === 'string' && orderId.trim() ? <Pressable onPress={() => router.push(`/safepay/checkout/${orderId.trim()}`)} className="flex-row items-center gap-1.5"><FileText size={14} color="#122A1C" /><Text className="text-[0.8125rem] font-semibold text-[#122A1C]">Se kontrakt</Text></Pressable> : null}</View>
    ...
    {canDispute ? <Pressable onPress={() => setDisputeOpen(true)} className="mt-4 rounded-full border border-[#B4453A] px-4 py-3"><Text className="text-center text-[0.8125rem] font-semibold text-[#B4453A]">Noe gikk galt? Opprett en tvist</Text></Pressable> : null}{activeDispute ? <View className="mt-4 rounded-2xl bg-[#FBF4F2] p-4"><Text className="text-center text-[0.8125rem] font-semibold text-[#B4453A]">Tvist pågår — admin gjennomgår saken</Text></View> : null}
    ...
    {disputeOpen ? <View className="mt-4 rounded-2xl border border-[#E6E7E1] bg-white p-5"><Text className="mb-3 text-[1rem] font-bold text-[#0B0B0B]">Opprett en tvist</Text><Text className="mb-2 text-[0.75rem] text-[#63665F]">Admin vil gjennomgå saken og kontakte begge parter.</Text>{DISPUTE_REASONS.map((reason) => <Pressable key={reason.value} onPress={() => setReasonCategory(reason.value)} className={['mb-2 rounded-xl border p-3', reasonCategory === reason.value ? 'border-[#2E6641] bg-[#EAF1E9]' : 'border-[#E6E7E1]'].join(' ')}><Text className="text-[0.8125rem] text-[#0B0B0B]">{reason.label}</Text></Pressable>)}<TextInput value={disputeTitle} onChangeText={setDisputeTitle} placeholder="Tittel (min. 5 tegn)" placeholderTextColor="#9B9E96" className="mb-2 rounded-xl border border-[#E6E7E1] p-3 text-[0.8125rem]" /><TextInput value={disputeDescription} onChangeText={setDisputeDescription} placeholder="Beskrivelse (min. 20 tegn)" placeholderTextColor="#9B9E96" multiline className="min-h-[100px] rounded-xl border border-[#E6E7E1] p-3 text-[0.8125rem]" /><View className="mt-3 flex-row gap-2"><Button label="Avbryt" variant="secondary" onPress={() => setDisputeOpen(false)} fullWidth /><Button label={disputeMutation.isPending ? 'Sender...' : 'Send tvist'} onPress={submitDispute} disabled={disputeMutation.isPending} fullWidth /></View></View> : null}
