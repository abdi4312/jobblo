...
const AddContributorModal: React.FC<AddContributorModalProps> = ({ listId, isOpen, onClose }) => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    setIsSearching(true);
    try {
      const response = await mainLink.get(`/api/users/search?query=${encodeURIComponent(searchQuery.trim())}`);
  ...
  const toggleUserSelection = (user: any) => {
    setSelectedUsers(prev => {
      ...
      if (isAlreadySelected) {
        ...
        if (prev.length >= 10) {
          toast.error("You can select up to 10 users at a time");
          return prev;
        }
        return [...prev, user];
      }
  ...
  const handleAddContributors = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["favoriteLists", "user", listId] });
      toast.success(`${selectedUsers.length} contributors added successfully`);
      ...
      >
        ...
        <div className="p-6 space-y-6 min-h-[500px] flex flex-col">
          ...
          <form onSubmit={handleSearch} className="flex gap-3 items-center">
            ...
            <button
              type="submit"
              className="px-8 py-3.5 bg-[#FF8A71] text-black font-bold rounded-[20px] hover:bg-[#ff7659] transition-all"
            >
              Search
            </button>
          </form>
          ...
          <div className="flex-1 overflow-y-auto -mx-2 px-2 space-y-1">
            {isSearching ? (
              <div className="flex justify-center py-10">
                <div className="animate-spin w-8 h-8 border-4 border-gray-100 border-t-[#FF8A71] rounded-full" />
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-7 h-7 rounded-full border-2 flex items-center justify-center transition-all ${isSelected
                      ? "border-[#7D1D54] bg-white"
                      : "border-gray-300 bg-white"
                      }`}>
                      {isSelected && (
                        <div className="w-4 h-4 rounded-full bg-[#7D1D54]" />
                      )}
                    </div>
                  </button>
                );
          ...
          <button
            ...
            className="w-full py-4.5 bg-[#FF8A71] text-black font-bold rounded-[22px] hover:bg-[#ff7659] disabled:bg-[#FF8A71]/40 disabled:text-gray-500 transition-all shadow-md active:scale-[0.98] text-lg mt-4"
          >
            Add contributor {selectedUsers.length > 0 && `(${selectedUsers.length})`}
          </button>
        </div>
      </div>
    </div>
