  ...
  return (
    <div
      className={`box-card-custom flex flex-col rounded-3xl p-8 transition-all duration-300 relative ${
        isPopular ? 'ring-2 ring-custom-green/50 shadow-2xl scale-105 z-10' : 'box-card-custom'
      }`}
    >
      {isPopular && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-orange-custom px-6 py-1.5 rounded-full shadow-lg">
          <p className="text-sm font-bold text-white uppercase tracking-wider">Most Popular</p>
        </div>
      )}

      <div className="mb-8">
        <h3 className="text-2xl font-bold text-custom-black mb-2">{plan.name}</h3>
        <p className="text-base text-custom-black/70">For serious freelancers</p>
      </div>

      <div className="mb-8">
        <div className="flex items-baseline gap-1">
          <span className="text-5xl font-black text-custom-black">{plan.price}</span>
      ...
      <div className="flex-1">
        ...
        <ul className="space-y-4">
          {plan.featuresText?.map((feature, i) => (
            <li key={i} className="flex items-start gap-3 text-[15px] text-custom-black/70">
