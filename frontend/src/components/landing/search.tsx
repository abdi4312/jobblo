...
function SearchComponent() {
  ...
  const { data: favoriteLists, isLoading: isListsLoading } = useFavoriteLists(user?._id);

  // Fetch all categories
  const { data: categories, isLoading: isCategoriesLoading } = useCategories();

  // Unified Search for Top results (Categories, People, Lists)
  const { data: searchResults, isLoading: isSearching } = useUnifiedSearch(searchQuery);
  ...
  const infiniteResults = infiniteData?.pages.flatMap((page) => page.results) || [];

  // Old search hook (kept for compatibility)
  useSearchUsers(activeTab === 'People' && searchQuery.length >= 2 ? searchQuery : undefined);
