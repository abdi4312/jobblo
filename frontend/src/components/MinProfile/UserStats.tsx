...
export const UserStats = ({ user }: UserStatsProps) => {
  ...
  const stats = [
    ...
    {
      label: "Tjent",
      value: `${user.earnings || 0} kr`,
      color: "text-custom-green",
    },
    {
      label: "Brukt",
      value: `${user.spending || 0} kr`,
      color: "text-custom-green",
    },
  ];
    ...
    <div className="w-full flex justify-center items-center flex-col gap-4">
      ...
      <div className="w-full grid mx-auto grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          ...
          >
            <div className="text-sm text-custom-green font-medium">
              {stat.label}
            </div>

            <div
              className={`text-[20px] font-bold text-custom-black leading-9.5 flex justify-end items-end`}
            >
              {stat.value}
            </div>
          </div>
      ...
      {user.bio && (
        <div className="flex flex-col gap-2.5 w-full px-6 py-5.5 bg-[#FFFFFF1A] rounded-xl shadow-sm">
          <div className="font-medium text-custom-green text-sm mb-1 tracking-wide">
            Bio
          </div>
      ...
      <div className="flex gap-2 flex-wrap w-full">
        {user.role && (
          <span className="px-4 py-1.5 bg-custom-green text-white rounded-full text-xs font-bold capitalize shadow-sm border border-white/10">
            {user.role}
          </span>
        )}
        {user.subscription && (
