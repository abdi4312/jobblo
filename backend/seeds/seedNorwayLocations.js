...
function walkSSBData(value, found = []) {
  if (Array.isArray(value)) {
    ...
    const name = value.name || value.navn || value.title || value.label || value.presentationName;
...
async function seedLocations() {
  try {
    ...
    const kartverketResponse = await fetch(`${KARTVERKET_BASE_URL}/fylkerkommuner`);
