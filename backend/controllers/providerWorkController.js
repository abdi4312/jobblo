...
exports.getOrderDetail = async (req, res) => {
  try {
    ...
    const calculation = {
      ...
      fee,
      total: (order.agreedPrice || 0) + fee,
      providerNet: (order.agreedPrice || 0) - fee,
    };
    const dispute = await Dispute.findOne({
      orderId,
      status: { $nin: ['resolved', 'closed', 'cancelled'] },
    }).select('status reasonCategory title openedAt');
...
exports.startJob = async (req, res) => {
  try {
    ...
    if (String(order.providerId) !== String(userId))
      return res.status(403).json({ error: 'Kun utfører kan starte jobben' });
    if (order.status === 'in_progress') return res.json({ message: 'Allerede startet', order });
    if (order.paymentStatus !== 'paid')
      return res.status(400).json({ error: 'Betaling må bekreftes først' });
    if (await hasActiveDispute(orderId)) return res.status(400).json({ error: 'Aktiv tvist' });
    if (order.status !== 'paid')
      return res
        .status(409)
        .json({ error: `Ugyldig statusovergang: ${order.status} → in_progress` });
    const now = new Date();
      ...
      { _id: orderId, status: 'paid' },
      {
        ...
        $push: {
          history: {
            ...
            userId,
            timestamp: now,
            data: { message: 'Utfører startet oppdraget' },
          },
        },
      },
      { new: true }
    ...
    if (updated.chatId) {
      ...
      if (chat) {
        ...
        chat.messages.push({
          type: 'system_status',
          systemData: { event: 'job_started', orderId: updated._id },
          text: 'Utfører har startet oppdraget',
          createdAt: now,
        });
        await chat.save();
      }
...
exports.updateProviderChecklist = async (req, res) => {
  try {
    ...
    if (!['paid', 'in_progress', 'ready_for_review'].includes(order.status))
      return res.status(400).json({ error: 'Ugyldig status' });
    const idx = order.checklist.findIndex((i) => i.id === itemId);
    if (idx === -1) return res.status(404).json({ error: 'Element ikke funnet' });
    if (order.checklist[idx].providerCompleted === Boolean(providerCompleted))
      return res.json({ message: 'Ingen endring', order });
    const now = new Date();
    const upd = {
      [`checklist.${idx}.providerCompleted`]: Boolean(providerCompleted),
      [`checklist.${idx}.providerCompletedAt`]: providerCompleted ? now : null,
      [`checklist.${idx}.providerCompletedBy`]: providerCompleted ? userId : null,
      [`checklist.${idx}.checked`]: Boolean(providerCompleted),
    };
...
exports.updateCustomerChecklist = async (req, res) => {
  try {
    ...
    if (String(order.customerId) !== String(userId))
      return res.status(403).json({ error: 'Kun oppdragsgiver' });
    if (!['ready_for_review', 'in_progress'].includes(order.status))
      return res.status(400).json({ error: 'Ugyldig status' });
    const idx = order.checklist.findIndex((i) => i.id === itemId);
    if (idx === -1) return res.status(404).json({ error: 'Element ikke funnet' });
    if (order.checklist[idx].customerConfirmed === Boolean(customerConfirmed))
      return res.json({ message: 'Ingen endring', order });
    const now = new Date();
    const upd = {
      [`checklist.${idx}.customerConfirmed`]: Boolean(customerConfirmed),
      [`checklist.${idx}.customerConfirmedAt`]: customerConfirmed ? now : null,
      [`checklist.${idx}.customerConfirmedBy`]: customerConfirmed ? userId : null,
    };
    const updated = await Order.findByIdAndUpdate(orderId, { $set: upd }, { new: true });
...
exports.markReadyForReview = async (req, res) => {
  try {
    ...
    if (order.paymentStatus !== 'paid')
      return res.status(400).json({ error: 'Betaling ikke bekreftet' });
    if (await hasActiveDispute(orderId)) return res.status(400).json({ error: 'Aktiv tvist' });
    if (order.status !== 'in_progress')
      return res
        .status(409)
        .json({ error: `Ugyldig statusovergang: ${order.status} → ready_for_review` });
    const now = new Date();
    const updated = await Order.findOneAndUpdate(
      { _id: orderId, status: 'in_progress' },
      {
        $set: { status: 'ready_for_review', readyForReviewAt: now },
        $push: {
          history: {
            action: 'ready_for_review',
            userId,
            timestamp: now,
            data: { message: 'Jobb meldt ferdig' },
          },
        },
      },
      { new: true }
    ...
    if (updated.chatId) {
      ...
      if (chat) {
        chat.messages.push({
          type: 'system_status',
          systemData: { event: 'ready_for_review', orderId: updated._id },
          text: 'Utfører melder jobben som ferdig',
          createdAt: now,
        });
        await chat.save();
      }
...
exports.reconcilePayment = async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ error: 'Ordre ikke funnet' });
    if (String(order.customerId) !== String(userId))
      return res.status(403).json({ error: 'Kun oppdragsgiver kan verifisere betaling' });
    ...
    if (!order.checkoutSessionId)
      return res
        .status(400)
        .json({ status: 'no_session', message: 'Ingen betalingssesjon funnet' });
    ...
    let session;
    try {
      session = await stripe.checkout.sessions.retrieve(order.checkoutSessionId);
    } catch (e) {
      return res
        .status(503)
        .json({ status: 'stripe_unavailable', message: 'Stripe ikke tilgjengelig' });
    }
    if (session.metadata?.orderId !== String(orderId))
      return res.status(400).json({ status: 'session_mismatch' });
    if (session.payment_status !== 'paid')
      return res.json({ status: 'not_paid', payment_status: session.payment_status });
    const updated = await Order.findOneAndUpdate(
    ...
    try {
      ...
      if (e.code === 11000) payment = await Payment.findOne({ orderId });
      else throw e;
    }
    const io = req.app.get('io');
    if (io) {
      io.to(`user_${order.providerId}`).emit('payment_confirmed', { orderId });
      io.to(`user_${order.customerId}`).emit('payment_confirmed', { orderId });
    }
    // Sync service status to paid (when payment is confirmed)
    const orderForService = updated || order;
    if (orderForService) {
      await Service.findByIdAndUpdate(orderForService.serviceId, { status: 'paid' });
    }
    res.json({ status: 'confirmed', order: updated || order, payment });
  } catch (err) {
    res.status(500).json({ error: 'Serverfeil', status: 'error' });
  }
};
