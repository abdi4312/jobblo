...
function googleCallback(req, res, next) {
  passport.authenticate('google-web', { session: false }, async (err, user, info) => {
    ...
    if (!user) {
      return res.redirect(oauthDestination({ req, platform: 'web', error: info?.code || 'google_failed' }));
    }

    if (user.isDeleted || user.accountStatus === 'deactivated') {
      return res.redirect(oauthDestination({ req, platform: 'web', error: 'account_deactivated' }));
    }
...
router.get('/vipps', optionalAuthenticate, (req, res) => vippsController.redirectToVipps(req, res, { platform: 'web' }));
router.get('/vipps/callback', (req, res) => vippsController.vippsCallback(req, res, { platform: 'web' }));
